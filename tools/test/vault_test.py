...
class MlxGroup(unittest.TestCase):
    """~197 MB that only a machine running the MLX backend needs, so it is a
    separate group rather than part of VAULTED_FILES: folding it in would push
    it onto every machine's iCloud and every restore for a backend that is off
    by default."""

    def test_the_mlx_files_are_not_in_the_default_set(self):
        for rel in V.MLX_FILES:
            self.assertNotIn(rel, V.VAULTED_FILES)

    def test_restore_ignores_them_unless_asked(self):
        with tempfile.TemporaryDirectory() as tmp:
            rime, store = Path(tmp) / "rime", Path(tmp) / "vault"
            rime.mkdir(); store.mkdir()
            rels = {a.rel for a in V.plan_restore(rime, store)}
            self.assertTrue(rels.isdisjoint(V.MLX_FILES))

    def test_restore_includes_them_when_asked(self):
        with tempfile.TemporaryDirectory() as tmp:
            rime, store = Path(tmp) / "rime", Path(tmp) / "vault"
            rime.mkdir(); store.mkdir()
            rels = {a.rel for a in V.plan_restore(rime, store, include_mlx=True)}
            for rel in V.MLX_FILES:
                self.assertIn(rel, rels)

    def test_backup_is_symmetric_about_the_flag(self):
        # A group that restores but never backs up would leave the vault copy
        # to rot the first time one machine updated MLX.
        with tempfile.TemporaryDirectory() as tmp:
            rime, store = Path(tmp) / "rime", Path(tmp) / "vault"
            rime.mkdir(); store.mkdir()
            off = {a.rel for a in V.plan_backup(rime, store, machine="m")}
            on = {a.rel for a in V.plan_backup(rime, store, machine="m",
                                                   include_mlx=True)}
            self.assertTrue(off.isdisjoint(V.MLX_FILES))
            self.assertTrue(set(V.MLX_FILES).issubset(on))

    def test_all_three_runtime_files_are_carried(self):
        # libmlx and libjaccl are what the plugin links; mlx.metallib is the
        # kernel library libmlx loads. A missing metallib is not a degradation
        # -- MLX throws from inside its device setup -- so carrying two of the
        # three would be worse than carrying none.
        names = {rel.rsplit("/", 1)[-1] for rel in V.MLX_FILES}
        self.assertEqual(names, {"libmlx.dylib", "libjaccl.dylib", "mlx.metallib"})
