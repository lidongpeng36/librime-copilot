...
class PayloadFiles(InstallFixture):
    ...
    def test_the_reporter_is_no_longer_part_of_the_payload(self):
        """It moved to rime-copilot-clients, where the tmux plugin ships it
        alongside the hooks that invoke it. Installing a second copy here would
        recreate the very divergence that move removed -- and `status` no
        longer looks at any such copy, so a stale one would be invisible."""
        _write(self.source_root / "rime_ctx_report.sh", "#!/bin/sh\nreport\n",
               executable=True)
        self.assertNotIn("rime_ctx_report.sh", I.payload_files(self.source_root))
