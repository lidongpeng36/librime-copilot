...
| `wordhead.py` | generating `private/wordhead.txt`, the word-head prior's table, from the corpus |
| `cli.py` | orchestration |

Subcommands: `status`, `restore`, `backup`, `fetch`, `build`, `deploy`,
`update`, `install`, `clean`, `personal`, `wordhead`. A new machine builds `build_copilot`
...
`dict.json`, `private.dict.yaml`, `private/personal.dict.yaml` and
`private/wordhead.txt` are vaulted (`vault.py:VAULTED_FILES`) and move by
iCloud; `private.predict.db` and
...
- **`copilot/rerank/llm/margin`** (default 2.0) — how much better a candidate
  ...
  **Every accept rate in this section is the pre-2026-09-23 `sel_idx`-only
  measure, and it is biased up.** A promotion committed, deleted and retyped
  reads as accepted (`sel_idx: 0`); `analyze_telemetry.mark_retypes` found 118
  such among 988 promotions, putting true acceptance near 63%. The comparisons
...
### The word-head prior: single characters were scored against the words they begin

`copilot/rerank/llm/word_head/weight` (code default 0, shipped schema 1.0) adds
`weight * log(1 - mass(c))` to each candidate of an **all-single-character
window**, after length normalisation. `mass(c)` is how often the user's own
corpus uses `c` as the first character of a longer word, from
`private/wordhead.txt` (`rime-copilot wordhead`, vaulted, regenerated by
`update`). Zero decodes: it is a lookup, which is why it replaced word-end.

Why it exists, measured over 23 days / 988 promotions on two machines: the
scorer's P(现 | 你) counts 现在, 发现, 出现, so it promoted 现 over 先 54 times
and was wrong 50 of them. Promoted characters were more word-head-like than
the ones they displaced 338 times to 69 (p = 1.1e-43), and among single-char
promotions 44% of the word-head-ward ones were harmful against 20% of the
rest. Held out (λ picked on the earlier half), the prior moves net keystrokes
from +120 to +145; harmful promotions fall 29%; `先->现` goes from 50 harmful
to ~0.

Four things are load-bearing:

- **The gate is the window, not the candidate** (`wordhead::WindowPriors`).
  Any multi-character Han candidate turns the prior off for the whole window,
  because the measurement never applied it between a character and a word.
- **The table comes from the corpus, not jieba's dictionary**: +145 vs +137
  held out. Characters the corpus never shows get no penalty, and that beat a
  jieba fallback.
- **A segment the prior changed is census** (`telemetry_commit.cc`), or a
  prior-blocked promotion — which looks like a plain success — would be
  sampled 1 in 20 against its fully-recorded failures.
- **`llm.margin` on a v10 line carries the prior.** `margin - prior_delta` is
  the pre-v10 quantity.

It does not touch the ~60% of remaining harm that is word-vs-word (`部署` →
`不熟`/`不是`, `哪些` → `那些`): that is the model not knowing the user's
vocabulary. `llm.cand_w`/`cand_q` (v10) record Rime's own candidate weights so
phase 2 can ask whether Rime's ordering already carries the user's history —
the only live source that did, in the design's measurement, was the user
dictionary; the vaulted static lexicons (`custom`, `personal`) did not.

`same_span_only` already keeps characters and words from being scored
together (4,438 candidates dropped by span in the window, 100% of a different
character count); the dilution this fixes happens inside a single
character's own score. The record, scripts and every number here are kept
locally: `specs/2026-09-23-wordhead-prior-design.md`,
`scripts/2026-09-23-wordhead-prior/`.

**What v10 records about it, and where it is read.** Present only on a window
the prior was applied to: `llm.prior_delta`, `llm.prior_changed`, and
`llm.best_noprior` -- what the model would have promoted without the prior,
`""` for nothing. `llm.best` is the pick *after* the prior, so a promotion the
prior blocked is recorded nowhere else. `analyze_telemetry.py` prints a
**"Word-head prior (v10)"** section after the paired promotions: segments the
prior BLOCKED (user kept the head = prior right; took `best_noprior` = prior
wrong; other) and segments it CREATED (accepted / rejected). Both are census,
so no weighting. A blocked segment whose kept head was deleted and retyped to
the blocked challenger counts as prior wrong (`ev.prior_retyped_to`, the
mirror of `retyped`); a prior-caused decline is its own `decline_split` kind,
`prior`, not `agreed`, which would have pointed at a retrain.
`mark_retypes` also rewrites a retyped promotion's `llm_verdict` to
`rejected`, so every LLM accept rate the report prints is retype-corrected --
on the 2026-08-31 -> 09-24 log the LLM path's headline reads 59.5%, not the
sel_idx-only figure.

**`rime-copilot status` saying `wordhead: ok` does not mean the prior is
running.** It proves the table loads (it counts the entries the C++ parser
would accept) and that the schema sets the weight -- nothing about the dylib.
A machine that restored the vault and never rebuilt has both and runs a
plugin with no prior at all. The check is the running image: the last line of
`private/copilot_telemetry/<machine>.jsonl` shows `"v":10`, and an engaged
all-single-character window carries `prior_delta`; or

```sh
strings "/Library/Input Methods/Squirrel.app/Contents/Frameworks/rime-plugins/librime-copilot.dylib" \
  | grep -c "word-head prior on"
```

is at least 1. (The plugin also logs `word-head prior on` each time the
filter is built -- a new session, a redeploy -- but only with
`rerank/llm/enable` and the weight both on.) Zero means the file predates the
prior, whatever `status` says.
...
| `*.custom.yaml`, the lexicon, `dict.json`, `private.dict.yaml` + `private/personal.dict.yaml`, `private/wordhead.txt`, the model | the vault, via `backup` / `restore` |
...
rime-copilot update                          # regenerates private/personal.dict.yaml
                                             # and private/wordhead.txt (both refuse
                                             # without a corpus), then rebuilds and deploys
